...
| `model.predict` (horizon) | Cryptic shape errors on a mismatched future length | The forecast horizon was never validated against the future frame | `predict` accepts 1..`prediction_length` periods per location (a chap eval backtest may request fewer) and rejects only futures longer than the trained horizon |
